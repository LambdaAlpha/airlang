...
- 若 `pattern` 为 `equiv(b)`（`inverse(b)`，`generate(b)`，`abstract(b)`），则匹配 `equiv(v)`（`inverse(v)`，`generate(v)`，`abstract(v)`），递归
  - 将 `v` 与 `b` 匹配绑定，得到 `cv`
  - 输出 `equiv(cv)`（`inverse(cv)`，`generate(cv)`，`abstract(cv)`）
...
逆模式：`{map : {variables : {map : {} : {symbol : ..} : .}} : {symbol : ..}}`
