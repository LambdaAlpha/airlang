...
若 i 为 `name : const`，且 `name` 已绑定值，则设置 `name` 的不变性为比特 `const`。若所设置的值会破坏上下文所保证的不变性，则设置失败。
...
若 i 为 `name : r`，且 `name` 已绑定值，则设置 `name` 的变量绑定逆向为比特 `r`。若所设置的值会破坏上下文所保证的不变性，则设置失败。
...
- variables 的键值对为 `bind : value`，`bind` 的一般形式为 `name . options`。
  - `options` 是一些选项，用于指定变量访问类型等。
  - `bind` 也可以省略 `options`：`name`，表示使用默认的选项。
  - 按照 `options` 选项将 `value` 绑定到键 `name` 上。
- reverse：逆向
- solver：求解器
